...
\begin{document}
	...
	\begin{sol}
		...
		Sia $y_n \in \text{Ran}(T)$, $n \in \mathbb{N}$, e supponiamo che $y_n \to y$ per $n \to +\infty$. Dobbiamo dimostrare che $y \in \text{Ran}(T)$. Per ipotesi:
		\begin{equation}
			y_n = Tx_n = Ax_n - x_n \tag{*}
		\end{equation}
		per una certa successione $\{x_n\}_{n \in \mathbb{N}} \in \HH$. Senza perdita di generalità possiamo assumere $x_n \in \text{Ker}(T)^\perp$, eventualmente eliminando dalla successione la componente che si proietta su $\text{Ker}(T)$.
		L'affermazione è dimostrata se riusciamo a mostrare che la successione $\{x_n\}$ è limitata: infatti, essendo $A$ compatto, esisterà una sottosuccessione $x_{n_k}$ tale che $Ax_{n_k} \to y' \in \HH$ per $k \to \infty$. Sostituendo in (*) concludiamo che $x_{n_k} \to x$ per un certo $x \in \HH$ per $k \to +\infty$. Per la continuità di $A$, $Tx = Ax - x = y$, dunque $y \in \text{Ran}(T)$.
		
		Procederemo per assurdo, assumendo che $\{x_n\}_{n \in \mathbb{N}} \subset \text{Ker}(T)^\perp$ sia illimitata. Se così non fosse, esisterebbe una sottosuccessione $x_{n_m}$ con $0 < \|x_{n_m}\| \to +\infty$ per $m \to +\infty$. Poiché i $y_n$ formano una successione convergente, e quindi limitata, dividendo per $\|x_{n_m}\|$ in (4.63) si ottiene:
		\begin{equation}
			T \frac{x_{n_m}}{\|x_{n_m}\|} = A \frac{x_{n_m}}{\|x_{n_m}\|} - \frac{x_{n_m}}{\|x_{n_m}\|} = \frac{y_{n_m}}{\|x_{n_m}\|} \to \mathbf{0}. \tag{**}
		\end{equation}
		Ma $A$ è compatto e i vettori $\frac{x_{n_m}}{\|x_{n_m}\|}$ sono limitati, quindi possiamo estrarre un'ulteriore sottosuccessione $x_{n_{m_k}}/\|x_{n_{m_k}}\|$ tale che:
		$$
		\frac{x_{n_{m_k}}}{\|x_{n_{m_k}}\|} \to x' \in \HH \quad \text{e} \quad T \frac{x_{n_{m_k}}}{\|x_{n_{m_k}}\|} \to Tx' \quad \text{per } k \to +\infty.
		$$
		Da (**) deduciamo che $x' \in \text{Ker}(T)$. Per ipotesi $\frac{x_{n_{m_k}}}{\|x_{n_{m_k}}\|} \in \text{Ker}(T)^\perp$, e poiché $\text{Ker}(T)^\perp$ è chiuso, allora $x' \in \text{Ker}(T)^\perp$. Di conseguenza $x' \in \text{Ker}(T) \cap \text{Ker}(T)^\perp = \{\mathbf{0}\}$, in contraddizione con
		$$
	...
	\begin{sol}
		...
		Poiché $f \in C^\infty(\bar{I})$, la funzione è limitata sul dominio compatto $\bar{I}$ (per il teorema di Weierstrass), dunque $f \in L^2(I)$ con norma finita.
		...
		|F(h)| \le \norm{f}_{L^2} \norm{h}_{H^1}.
		\]
		Essendo $\norm{f}_{L^2}$ una costante finita, esiste $C = \norm{f}_{L^2}$ tale che $|F(h)| \le C \norm{h}_{H^1}$.
		Dunque il funzionale $F$ è limitato:
		\[
		\norm{F}_{(H^1_0)^*} \le \norm{f}_{L^2} < \infty,
		\]
		e appartiene allo spazio duale topologico $(H^1_0(I))^*$.
	...
	\section*{Traccia dell'Esercizio 5 $\psi(0)=0$}
